...
const schema = z.object({
  ...
  vehicle_year: z
    ...
    .refine((y) => parseInt(y) >= 2010 && parseInt(y) <= new Date().getFullYear() + 1, 'Enter a year between 2010 and now'),
...
export default function OnboardVehicleScreen() {
  ...
  const onSubmit = async (data: FormData) => {
    if (!selectedPlatform) {
      Alert.alert('Select a platform', 'Please select the platform you drive for.')
      return
    }
    ...
    >
      <ScrollView contentContainerStyle={styles.inner} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.section}>
          ...
          <View style={styles.platformRow}>
            {PLATFORMS.map(({ label, value }) => (
              <TouchableOpacity
                key={value}
                style={[styles.platformChip, selectedPlatform === value && styles.platformChipActive]}
                onPress={() => setSelectedPlatform(value)}
              >
                <Text style={[styles.platformChipText, selectedPlatform === value && styles.platformChipTextActive]}>
                  {label}
                </Text>
              </TouchableOpacity>
            ))}
        ...
        <View style={styles.form}>
          {([
            { name: 'vehicle_make' as const, label: 'Make', placeholder: 'Honda' },
            { name: 'vehicle_model' as const, label: 'Model', placeholder: 'Civic' },
            { name: 'vehicle_year' as const, label: 'Year', placeholder: '2022', keyboard: 'number-pad' as const },
            { name: 'license_plate' as const, label: 'License Plate', placeholder: 'ABCD 123' },
              ...
              <Controller
                ...
                render={({ field: { onChange, value } }) => (
                  <TextInput
                    ...
                    autoCapitalize={name === 'license_plate' ? 'characters' : 'words'}
