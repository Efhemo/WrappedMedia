...
export function CampaignForm() {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
      {[
        { name: 'name' as const, label: 'Campaign Name', placeholder: 'Spring 2026' },
        { name: 'brand' as const, label: 'Brand', placeholder: "Wendy's" },
        { name: 'markets' as const, label: 'Markets (comma separated)', placeholder: 'Toronto, Vancouver' },
        { name: 'driver_count' as const, label: 'Target Driver Count', placeholder: '50' },
      ].map(({ name, label, placeholder }) => (
        <div key={name}>
          <label className="text-slate-400 text-xs font-medium block mb-1">{label}</label>
          <input {...register(name)} placeholder={placeholder} className={inputClass(!!errors[name])} />
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="text-slate-400 text-xs font-medium block mb-1">Wrap Type</label>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="text-slate-400 text-xs font-medium block mb-1">Start Date</label>
          <input type="date" {...register('start_date')} className={inputClass(!!errors.start_date)} />
        </div>
        <div>
          <label className="text-slate-400 text-xs font-medium block mb-1">End Date</label>
      ...
      <div>
        ...
        <textarea {...register('notes')} rows={2} placeholder="Any special instructions..." className={`${inputClass(false)} resize-none`} />
      </div>
